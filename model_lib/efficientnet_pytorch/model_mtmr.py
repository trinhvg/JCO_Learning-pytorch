...
class EfficientNet(nn.Module):
    ...
    def __init__(self, task_mode='class', blocks_args=None, global_params=None):
        ...
        if self.task_mode in ['multi_mtmr',]:
            self.attribute_feature_fc = nn.Linear(out_channels, 256)
            self.regression_ = nn.Linear(256, 1)
            self.classifier_ = nn.Sequential(
                nn.Dropout(self._global_params.dropout_rate),
                nn.Linear(out_channels + 256, self._global_params.num_classes),
            )
        self._swish = MemoryEfficientSwish()
    ...
    def forward_once(self, inputs):
        ...
        if self.task_mode == 'class':
            ...
            return r_out[:, 0]
        elif self.task_mode == 'multi_mtmr':
            attribute_feature = self.attribute_feature_fc(x)
            r_out = self.regression_(attribute_feature)
            c_out = torch.cat([attribute_feature, x], dim=1)
            c_out = self.classifier_(c_out)
    ...
    def forward(self, input):
        ...
        cat_subtlety_score = torch.cat([attribute_score_1, attribute_score_2])

        return cat_output, cat_subtlety_score
...
def jl_efficientnet(task_mode='multi_mtmr', pretrained=True, num_classes=4, **kwargs):
    """
...
#     net = jl_efficientnet(task_mode='multi_mtmr', pretrained=True, num_classes=3)
#     net = torch.nn.DataParallel(net).cuda()
#     y_class, y_regres = net(torch.randn(48, 3, 512, 512).cuda())

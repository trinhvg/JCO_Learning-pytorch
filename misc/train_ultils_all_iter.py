...
def log_train_ema_results(engine, info):
    ...
    update_log(training_ema_output, engine.state.iteration, 'train-ema', 'green',
...
def process_accumulated_output_multi(output, batch_size, nr_classes):
    ...
    def uneven_seq_to_np(seq):
        ...
        for idx in range(0, len(seq) - 1):
            cat_array[idx * batch_size: (idx + 1) * batch_size] = seq[idx]
        idx = -1 if len(seq) == 1 else idx  # in case len(seq) ==1 then the for loop below will be skipped
...
def inference(engine, inferer, prefix, dataloader, info):
    ...
    update_log(output_stat, engine.state.iteration, prefix, 'red',
               info['tfwriter'], info['json_file'], info['logging'])
    return
